...
export interface Execution { id: string; case_id: string; replicate: number; status: string; output: Json; output_present: boolean; trace: Json; case: Obj; results: Result[]; reviews: Obj[]; target_latency_ms: number | null; evaluation_latency_ms: number | null; target_error: string | null; metadata: Obj; late_completion: boolean; attempts?: Obj[]; experiment_name?: string }
